...
void dashboard()
{
  ...
  display.setFont(&Roboto_Thin72pt7b);

  display.setTextColor(GxEPD_BLACK);
  int16_t tbx, tby;
  uint16_t tbw, tbh;
  display.getTextBounds("Loading...", 0, 0, &tbx, &tby, &tbw, &tbh);
...
const GFXfont *getFont(const char *name)
{
  if (strcmp(name, "robotothin72") == 0)
  {
    return &Roboto_Thin72pt7b;
  }
  if (strcmp(name, "azonix36") == 0)
  {
    return &Azonix36pt7b;
  }
  return &Roboto_Thin72pt7b;
...
//  // center HelloWorld
//  display.getTextBounds("Hellow worlds...", 0, 0, &tbx, &tby, &tbw, &tbh);
...
void drawStrings(LinkedList<PaperString> *strings)
{
  for (byte i = 0; i < strings->size(); i++)
  {
    PaperString s = strings->get(i);
    Serial.printf("%d (%d,%d)", i, s.x, s.y);
    Serial.println(s.message);
    display.setFont(getFont(s.font));
    display.setTextColor(getColor(s.color));
    display.setCursor(s.x, s.y);
    display.print(s.message);
  }
}

void drawRects(LinkedList<PaperRect> *rectangles)
{
  for (byte i = 0; i < rectangles->size(); i++)
  {
    PaperRect r = rectangles->get(i);
    const char *color_name = r.color;
    int color = getColor(color_name);
    display.fillRect(r.x, r.y, r.w, r.h, color);
  }
}

void drawDashboard(Dashboard dashboard)
{
  display.setRotation(dashboard.rotation);
  ...
  do
  {
    ...
    display.setFont(&Roboto_Thin72pt7b);
    display.setTextColor(GxEPD_BLACK);
    drawRects(dashboard.rectangles);
    drawStrings(dashboard.strings);
  } while (display.nextPage());
  //
  //  display.firstPage();
  //  do
...
//void getCentered(char* message, GFXfont* font) {
//  display.setFont(&Roboto_Thin72pt7b);
//  display.setTextColor(GxEPD_BLACK);
//
//  int16_t tbx, tby; uint16_t tbw, tbh;
